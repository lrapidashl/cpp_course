...
void ballsCollision(std::vector<Ball>& balls)
{
    for (size_t i = 0; i < balls.size(); i++)
    {
        for (size_t j = i + 1; j < balls.size(); j++)
        {
            ...
            {
                auto iMass = (float)((4.f / 3 * M_PI) * pow(balls[i].radius, 3));
                auto jMass = (float)((4.f / 3 * M_PI) * pow(balls[j].radius, 3));
                sf::Vector2f dPosition = getNormalVector(deltaPosition)
                                     * (std::abs(getVectorLength(deltaPosition) - (balls[j].radius + balls[i].radius)) / 2);
                float iSpeedProjection = scalar(balls[i].speed - balls[j].speed, balls[i].position - balls[j].position)
                        / (float)(getVectorLength(balls[i].position - balls[j].position));
                float jSpeedProjection = scalar(balls[j].speed - balls[i].speed, balls[j].position - balls[i].position)
                        / (float)(getVectorLength(balls[j].position - balls[i].position));
                balls[i].position -= dPosition;
                balls[j].position += dPosition;
                balls[i].speed -= (2 * jMass * jSpeedProjection + iSpeedProjection * (iMass - jMass)) / (iMass + jMass)
                        * getNormalVector(balls[i].position - balls[j].position);
                balls[j].speed -= (2 * iMass * iSpeedProjection + jSpeedProjection * (jMass - iMass)) / (iMass + jMass)
